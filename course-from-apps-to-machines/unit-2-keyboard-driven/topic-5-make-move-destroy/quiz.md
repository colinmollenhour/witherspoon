...
**Explanation:** `mkdir` creates only the last part of the path it is given, so plain
...
**Explanation:** The manual page describes `mv` as "Rename SOURCE to DEST, or move SOURCE(s) to
...
**Explanation:** The opposite happens. `draft.html` does not exist, so this is the *rename* case —
...
**Explanation:** `rm` succeeds silently, so no output is the *expected* result, not evidence that
...
**Sample answer:** The shell splits the line on whitespace, so `mv My Project first-site` passes three
...
**Explanation:** Whitespace is how the shell decides where one argument ends and the next begins, so a
