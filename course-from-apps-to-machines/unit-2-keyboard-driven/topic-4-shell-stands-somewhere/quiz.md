...
1. `/home/you/projects`
2. `/home/you/projects/first-site`
3. `/home/you`
4. Nothing — `cd -` already printed the directory, so `pwd` has no output

**Correct:** 2

**Explanation:** `cd ~/projects/first-site` lands in the project folder. `cd ..` moves one level up
...
1. The file is corrupted — that message is what a damaged file looks like
2. `index.html` does not start with `/`, so the shell looked for it inside `~/projects`, where no
   such file exists
3. `index.html` is a hidden file, so it must be revealed with `ls -a` before any command can read it
4. Relative paths work only for directories, so files must always be named with an absolute path

**Correct:** 2

**Explanation:** A path that does not begin with `/` is measured from the current working directory.
...
**Explanation:** `cd -` returns the shell to the directory it was in before the last `cd`, and prints
...
**Sample answer:** The prompt ending in `$` is bash, the default shell on Linux. The prompt ending in
...
**Explanation:** The last character of a stock prompt identifies the shell: bash's `\$` escape renders
...
**Explanation:** The reasoning is backwards. A prompt shows the current directory only because it has
