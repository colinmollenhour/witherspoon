...
**Explanation:** `less` is a pager: it stops after each screenful, lets you move forward with Space,
...
**Explanation:** `grep -rn` output is three fields split on the *first two* colons: path, then line
...
**Sample answer:** It prints `7 /etc/hosts`. Bare `wc` gives three columns in a fixed order — lines,
...
**Explanation:** The three columns are lines, words, bytes, in that order, so reading the wrong
...
**Sample answer:** In the first command you named one single file, so `grep` leaves the `path:`
...
**Explanation:** The format looks unreliable and is not: the prefix appears exactly when it tells
...
**Explanation:** The opposite is true — nothing has hung. `less` is a full-screen program that has
