...
- `2`
- `first-site`
- The full `ls -la` listing, followed by `1`

**Correct option index:** 0

**Explanation:**

`ls -la ~/projects` prints a line for `first-site`, a line for `notes`, and lines
...
- `grep -rn "http" ~/projects | matches.txt`
- `grep -rn "http" ~/projects > matches.txt`
- `grep -rn "http" ~/projects | less`
- `grep -rn "http" ~/projects | wc -l`

**Correct option index:** 1

**Explanation:**

`>` is the one that makes a container: it creates `matches.txt` on disk, and the
...
It receives the text that `grep` printed — only the listing lines that contained
`site`, not the whole `ls` listing — arriving on `wc`'s standard input. It is not given a filename,
which is why it prints only a number instead of a number and a name the way `wc -l /etc/hosts`
prints `7 /etc/hosts`.
...
(1) it receives `grep`'s output, not `ls`'s original listing; (2) that text
arrives on stdin; (3) no file or filename is involved.

**Explanation:**

Each stage of a pipeline is handed the previous stage's stdout and nothing else, so
...
False — no file is written at any point, temporary or otherwise. Both programs run
...
- Nothing went wrong — no part of that command names a file, so nothing was written.
- `wc -l` deleted the temporary file when it finished; add `-l` to `ls` to see it.
- The result went to stderr instead of stdout, so it was never saved.
- The file is hidden, and `ls -a` will reveal it.

**Correct option index:** 0

**Explanation:**

A pipeline connects programs; it never creates a file, and only `>` or `>>` followed
