...
**Explanation:** `nano index.html` opens the file in the terminal window you are already in, and
...
**Explanation:** `.` is the current directory, so `code .` says *open this folder* — the editor loads
...
**Explanation:** The opposite is true — `^` is not a character you type at all. It is shorthand for
...
**Sample answer:** Nothing back from `which nano` means nano is not installed on this machine — `which`
...
**Explanation:** `which` reports whether a program exists on disk, so silence is a real answer, not an
