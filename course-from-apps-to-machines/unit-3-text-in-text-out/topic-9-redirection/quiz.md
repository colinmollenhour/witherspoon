...
**Explanation:** The third command uses `>`, which truncates `notes.txt` to empty before `wc` writes,
...
**Explanation:** A command has two output streams: stdout for results and stderr for messages about
...
**Sample answer:** `wc /etc/hosts >> notes.txt` — using `>` instead would have truncated `notes.txt` to
...
**Explanation:** `>>` appends and `>` truncates, and the only way to tell which you need is to ask
...
**Explanation:** `/dev/null` discards everything written to it, so the redirect runs normally and the
...
**Explanation:** It is the other way round from what most people expect: `> /dev/null` discards stdout,
