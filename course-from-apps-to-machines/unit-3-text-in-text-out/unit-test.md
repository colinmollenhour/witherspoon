...
`which` locates a *program* on disk. Silence is an answer: it means there is no nano
...
`.` is the current directory, so `code .` does not say "open a file" — it says *open
...
`>` starts a file from scratch. It creates the file if it is missing and empties it if
...
The opposite is true — the message appears on your screen, and `out.txt` ends up empty.
...
`/dev/null` is a destination that discards whatever is written to it, so `> /dev/null`
...
Nothing failed, and nothing was supposed to appear. `>` makes a **container**; `|`
...
`grep -rn "http" ~/projects | less`. The pipe hands `grep`'s standard output to
