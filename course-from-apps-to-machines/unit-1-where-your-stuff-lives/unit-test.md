...
**Assesses:** locating your home directory on macOS, Linux, and WSL and knowing why the Desktop is the
wrong place; naming what a desktop OS gives you that a tablet does not; reading a terminal prompt for
your username and your platform; classifying a path as absolute or relative and writing one out;
rewriting a location with `~`, `.`, and `..`; predicting case-sensitivity collisions across platforms;
knowing that renaming an extension leaves the bytes alone; reading every field of an `ls -la` line; and
telling a file apart from the program that opens it.

**Questions:** 10 — six multiple choice, two true/false, two short answer. Questions 3 and 10 are
synthesis questions and need two topics at once.
...
**Explanation:** Apple states the rule in one line: "`/Users`—This directory contains one or more user
...
**Explanation:** A desktop OS gives you three things a tablet withholds: two programs can work on the
...
**Explanation:** The stock prompt is built from `\u@\h:\w\$`, which renders as `colin@seamus:~$` — the
...
**Explanation:** It says the opposite. The zsh manual defines the escape as "%# — A '#' if the shell is
...
**Explanation:** One character decides this: the first one. The specification is blunt — "If the
...
**A grader must see:** (1) three distinct correct rewrites — a `~` form, a `.` form, and a `..` form —
...
**Explanation:** The specification grounds `~` directly: bare `cd` behaves "as if the directory named in
...
**Explanation:** The opposite is true — nothing inside the file moved. Renaming edits the name and only
...
**Explanation:** The fields run type character, three `rwx` triples, link count, owner, group, size in
...
**Explanation:** Opening one file twice removes every other variable — same path, same bytes, same size
...
**Sample answer:** (a) The leading `-` is the type character and says this is a regular file; a `d`
...
**A grader must see:** (1) all three fields named correctly — type character, link count, size in bytes;
...
**Explanation:** These two topics have to be held at once, because the listing is the evidence and the
