...
**Title:** Reading a URL as six labelled parts
**Description:** Labelling the parts of a real address, deciding when a port must be typed, working out what the far machine actually receives, and naming what a `file://` URL lacks.
**Passing score:** 70
...
- A. Your classmate — everything between `://` and the first `/` is the host, colon included.
- B. Nobody — a numeric address cannot be a host, so this URL has no host at all.
- C. **`192.168.1.42:8000` is the *authority*. The host is `192.168.1.42` and the port is `8000`, separated by a colon.**
- D. Your classmate — but only because the host here is an IP address; with a domain name the port would be part of the path.

**Correct:** C

**Explanation:** MDN describes "the _authority_, which is separated from the scheme by the character
...
- A. `https://www.example.com/index.html` — service listening on 443
- B. `http://www.example.com/index.html` — service listening on 80
- C. **`http://192.168.1.42/about/index.html` — service listening on 8000**
- D. `http://192.168.1.42:8000/about/index.html` — service listening on 8000

**Correct:** C

**Explanation:** The port "is usually omitted if the web server uses the standard ports of the HTTP
...
**Explanation:** The opposite is true — the fragment is the one visible part of the address that the far
...
- A. Their browser fetches your page across the network from your computer.
- B. **Their browser tries to open that path on their own disk, finds nothing there, and shows an error.**
- C. It works — the URL is absolute, and absolute URLs work from anywhere.
- D. Their browser prompts them for your machine's IP address, then fetches the page from it.

**Correct:** B

**Explanation:** A `file://` URL is an instruction to read a path on *the reader's own disk*, and your
...
**Sample answer:** The `file://` URL has no host, no port, and nothing listening. Both URLs have a
...
**Explanation:** Full credit needs the three empty slots *and* the reason they are empty. A learner who
