...
**Explanation:** The request line has the grammar `<method> <request-target> <protocol>`, so reading
...
**Explanation:** In `curl -v` output, `>` marks lines curl **sent** and `<` marks lines curl
...
**Sample answer:** The server must respond with a `400` (Bad Request) status code — RFC 9112 says a
...
**Explanation:** The common wrong answer is that the server falls back to a default site or returns
...
**Explanation:** `curl -I` sends a `HEAD` request, and the response comes back as head only — status
...
**Explanation:** The opposite is true — the empty line is what makes the message complete. MDN's
